...
The standard solution is to construct a resolution pyramid: keep a copy of each image at half-resolution, quarter-resolution, etc. and only send users the smallest one that doesn't need to be scaled at once. As the user zooms in and out, the site should smoothly transition between different levels as appropriate. A simple script can handle the process of creating the smaller versions of each image.
