...
- **[2026-09-17] Twenty code comments cite `accessibility-plan.md` sections that
  ...
  comment pass cleared `packages/map/nextRoom.ts` and five of the remaining in
  the hooks batch (`useMapCursor.ts`, `useModeTransition.ts`); the rest are
  still-queued files the pass reaches anyway, so this entry is for the two
  that a queue position would not catch:
