...
- **[2026-09-17] The canvas-side hover gold is written three times.** Separate
  from the entry above, which is about the DOM's different gold:
  `rgba(200,169,95, …)` appears as `center.ts`'s `HOVER_GLOW_FILL`/`_STROKE`,
  `render.ts`'s `FAVORITE_HOVER_GLOW_FILL`/`_STROKE`, and `gl/glowTexture.ts`'s
