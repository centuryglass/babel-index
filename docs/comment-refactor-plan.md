...
The search-half batch (2026-09) showed the worst variant: prose that restates an
*enumeration or formula* computed nearby goes quietly stale when the code grows.
`scoring.ts` said "five-constant" for a seven-constant weights shape, "three
more independent sorts" for four axes, listed three axes where `explainRanking`
reports four, and carried a soft-OR formula missing the `Kt` factor the code
multiplies; a dead symbol name (`explainScore`) had propagated into three files
at once. When a comment counts, lists, or restates a formula, check it against
the declaration in the same breath, and prefer naming the symbol over the count.
...
Progress so far: **9 / 107 source files** (`main.tsx`, `illusion.ts`,
`board.ts`, `moves.ts`, `nextRoom.ts`, `prng.ts`, `ordering.ts`, `scoring.ts`,
`searchResult.ts`), plus 5 paired tests (`illusion.test.ts`, `board.test.ts`,
`nextRoom.test.ts`, `ordering.test.ts`, `scoring.test.ts`) — all ticked in the
