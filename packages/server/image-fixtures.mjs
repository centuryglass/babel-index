/**
 * Synthetic image headers, for testing the parsers in scan.mjs.
 *
 ...
 * header.
 *
 * @param {number[]} [opts.before] markers to emit before the frame header, so
 *   the walk is exercised rather than a fixed offset. 0xc4 (DHT) shares the
 *   SOF marker range and must be skipped rather than read as a frame.
 * @param {number} [opts.marker] the frame header: 0xc0 baseline, 0xc2 progressive.
 */
export function jpeg(w, h, { before = [0xe0], marker = 0xc0 } = {}) {
