...
test('favoriteSort in relevance mode passes strength through by identity', () => {
  const strength = new Float32Array([0.9, 0.4, 0.1, 0]);
  const base = [3, 1, 0, 2];
...
test('favoriteSort in relevance mode with no search returns null strength [SR-26]', () => {
  const base = [3, 1, 0, 2];
  const result = favoriteSort(base, { mode: 'relevance', files, counts: {}, mine: new Set() });
...
test('favoriteSort with no search boosts favorites to 1 and leaves the rest at 0 [SR-28]', () => {
  ...
  assert.deepEqual(Array.from(result.strength), [1, 1, 0, 0]);
});
...
test('favoriteSort composes the boost with a running search rather than replacing it', () => {
  ...
  const search = { order: base, strength: new Float32Array([0.1, 0.8, 0.05, 0.05]) };
  ...
  // own search strength.
  const byId = new Map(result.order.map((id, i) => [id, result.strength[i]]));
...
test('favoriteSort strength is monotone non-increasing with rank', () => {
  const base = [0, 1, 2, 3];
  const search = { order: base, strength: new Float32Array([0.9, 0.2, 0.95, 0.1]) };
  const result = favoriteSort(
    base,
    { mode: 'count', files, counts: { 'a.jpg': 5, 'c.jpg': 3 }, mine: new Set() },
    search
  );
  for (let i = 1; i < result.strength.length; i++) {
    assert.ok(result.strength[i] <= result.strength[i - 1], `rank ${i} must not exceed rank ${i - 1}`);
  }
});
...
test('favoriteSort in random mode shuffles order but passes strength through untouched', () => {
  const strength = new Float32Array([0.9, 0.4, 0.1, 0]);
  const base = [3, 1, 0, 2];
  const result = favoriteSort(
    base,
  ...
  assert.equal(result.strength, strength, 'a shuffle carries no confidence claim, so strength passes by identity');
