    ...
    for (let gy = bounds.y0; gy <= bounds.y1; gy++) {
      for (let gx = bounds.x0; gx <= bounds.x1; gx++) {
        ...
        if (favorites && !isCtr && !isGeneric) {
          ...
          drawFavoriteBadge(
            ctx, cache, favorites.isFavorite(order[rank]) ? FAV_ON : FAV_OFF, cellPx, sx, sy, level, hovered
          );
        }
...
export function drawFavoriteBadge(
  ...
  if (hit && hit.level === level) {
    const { x, y, w, h } = favoriteIconScreenRect(cellPx, sx, sy, naturalIconSize(hit), level);
    if (hit.rect) {
      const { sx: rx, sy: ry, sw, sh } = hit.rect;
